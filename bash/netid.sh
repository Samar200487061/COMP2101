...
echo " --- Samar Rana --- Bash Lab 4 ---"

################
# Data Gathering
################

# COMMAND LINE ARGUMANETS
robin=""
while [ $# -gt 0 ]; do
    case "$1" in
    -v )
        ...
        ;;
    *)
        # Assign every other cmdline arg. to $robin if $robin is empty
        [ "$robin" == "" ] && robin="$1"
        ;;
    esac
...
# interface="eno1"
if [ "$robin" == "" ]; then
    echo "--- no cmdline argument with interface passed ---"
else
...
#####
echo "--- Task 2 ---"
arr=$(nmcli device status | awk '{print $1}' | awk '(NR>1)' | head -n -1)
